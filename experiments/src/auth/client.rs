...
impl HaloAuthClient {
    ...
    pub fn from_xbox_client<P: XblAuthProvider + 'static>(
        ...
        Self::from_xbox_client_with_timeout(xbox, DEFAULT_AUTH_TIMEOUT)
    }

    /// Builds Waypoint authentication with a custom per-request timeout.
    ///
    /// Applies to Spartan-token acquisition, current-user lookup, and clearance
    /// acquisition. [`Self::from_xbox_client`] uses 10 seconds. Xbox sign-in and
    /// XSTS requests retain the timeouts controlled by the `xbox` crate.
    /// Configure Halo API request timeouts separately with
    /// [`crate::clients::hi::HaloInfiniteClientBuilder::timeout`].
    pub fn from_xbox_client_with_timeout<P: XblAuthProvider + 'static>(
        xbox: impl Into<Arc<XboxClient<P>>>,
        timeout: Duration,
    ) -> Self {
        Self::from_xbox_client_with_endpoints_and_timeout(xbox, &AuthEndpoints::default(), timeout)
    }
    ...
    pub(crate) fn from_xbox_client_with_endpoints<P: XblAuthProvider + 'static>(
        ...
        Self::from_xbox_client_with_endpoints_and_timeout(xbox, endpoints, DEFAULT_AUTH_TIMEOUT)
    }

    /// Builds the authentication flow with overridable URLs for crate tests.
    pub(crate) fn from_xbox_client_with_endpoints_and_timeout<P: XblAuthProvider + 'static>(
        xbox: impl Into<Arc<XboxClient<P>>>,
        endpoints: &AuthEndpoints,
        timeout: Duration,
    ) -> Self {
